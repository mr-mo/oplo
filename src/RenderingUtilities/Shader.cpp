...
{
	return Compile( target_, filenames_ );
}
...
bool Shader::Compile( Target target, const std::string& fn )
{
	std::vector< std::string > fn_vector;

	fn_vector.push_back( fn );

	return Compile( target, fn_vector );
}

bool Shader::Compile( Target target, const std::vector< std::string >& fn )
{
	Destroy();
	...
	id_ = glCreateShader( static_cast< GLenum >( target ) );
	...
	for( unsigned i = 0 ; i < fn.size(); ++i )
	{
		...
		success = LoadFile( fn[i], vSource.back() );
	...
	if( !success )
	{
		std::cout << "Couldn't find file " << fn[failed] << std::endl;
	}
...
GeometryShader::GeometryShader( const GeometryShader& rhs ) : Shader( rhs )
{
	Destroy();
	input_type_ = rhs.input_type_;
	output_type_ = rhs.output_type_;
	output_count_ = rhs.output_count_;
}

GeometryShader& GeometryShader::operator=( const GeometryShader& rhs )
{
	if( this != &rhs )
	{
		Destroy();
...
bool Compile( Shader::Target target, const std::string& fn )
{
	return false;
}

bool GeometryShader::Compile( Target target, unsigned input_type, unsigned output_type, unsigned output_count, const std::vector< std::string >& fn )
{
	bool r = Shader::Compile( target, fn );
...
bool GeometryShader::Compile( Target target, unsigned input_type, unsigned output_type, unsigned output_count, const std::string& fn )
{
	bool r = Shader::Compile( target, fn );
	input_type_ = input_type;
	output_type_ = output_type;
...
Program::Program() : program_( 0 ), transform_feedback_type_( GL_SEPARATE_ATTRIBS )
{ }
